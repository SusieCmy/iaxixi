    ...
    if (error && !data) {
        return (
            <div className="flex h-[400px] w-80 flex-col items-center justify-center gap-2 rounded-3xl bg-base-200 shadow-xl text-base-content/50">
                ...
                <p>{error}</p>
                <button
                    onClick={() => fetchNews(sourceId)}
                    className="mt-2 rounded-full bg-base-300 px-4 py-1.5 text-xs font-medium transition-colors hover:bg-base-300/80"
                >
                    Retry
                </button>
            </div>
                ...
                <div className="flex items-center justify-between p-6 text-white">
                    <div className="relative">
                        ...
                        >
                            ...
                            <span className="font-semibold tracking-wide text-sm">{currentSource?.name}</span>
                        ...
                        {showSources && (
                            <>
                                ...
                                />
                                <div className="absolute left-0 top-full z-20 mt-2 w-40 overflow-hidden rounded-xl bg-white/90 p-1 shadow-xl backdrop-blur-xl dark:bg-slate-800/90">
                                    {NEWS_SOURCES.map((source) => (
                                        <button
                                            ...
                                            className={cn(
                                                ...
                                                sourceId === source.id ? "bg-rose-500 text-white hover:bg-rose-600" : "text-slate-700 dark:text-slate-200"
                                            )}
                                        >
                                            {source.name}
                                        </button>
                        ...
                        <div className="space-y-3">
                            {data?.items.map((item, index) => (
                                <a
                                    ...
                                    className="block rounded-xl border border-base-200 bg-base-50/50 p-3 transition-all hover:-translate-y-0.5 hover:border-rose-200 hover:bg-rose-50 hover:shadow-md dark:hover:border-rose-900/30 dark:hover:bg-rose-900/10"
                                >
