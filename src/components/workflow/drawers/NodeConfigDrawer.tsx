  ...
  return (
    <>
      ...
      >
        {/* 弹窗主体 - 阻止点击事件冒泡 */}
        <div
          className="relative w-full max-w-lg rounded-2xl border border-(--jp-mist)/50 bg-(--jp-cream) shadow-2xl transition-all"
        ...
        >
          ...
          <div className="flex items-center justify-between border-(--jp-mist) border-b p-6">
            <div>
              ...
              <p className="text-(--jp-ink)/60 text-sm">ID: {node.id}</p>
            </div>
            <button
              onClick={onClose}
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-lg border border-(--jp-mist) bg-(--jp-cream) px-4 py-3 transition-all focus:border-(--jp-vermilion) focus:outline-none focus:ring-2 focus:ring-primary/20"
                  placeholder="请输入节点名称"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={4}
                  className="w-full resize-none rounded-lg border border-(--jp-mist) bg-(--jp-cream) px-4 py-3 transition-all focus:border-(--jp-vermilion) focus:outline-none focus:ring-2 focus:ring-primary/20"
              ...
              {/* 异常处理开关 - 仅对普通节点显示 */}
              {node.type === 'default' && (
                <div className="flex items-center justify-between rounded-lg border border-(--jp-mist) p-4">
                  <div>
                    <div className="font-medium text-sm">异常处理</div>
                    <div className="text-(--jp-ink)/60 text-xs">
                      开启后可配置成功和失败的不同执行路径
                    </div>
                  </div>
                  <input
          ...
          {/* 底部操作栏 */}
          <div className="flex items-center gap-3 border-(--jp-mist) border-t p-6">
            <button
              onClick={onClose}
              className="flex-1 rounded-lg border border-(--jp-mist) px-4 py-3 transition-all hover:bg-(--jp-paper)"
            >
              取消
            </button>
            <button
              onClick={handleSave}
              className="flex-1 rounded-lg bg-(--jp-vermilion) px-4 py-3 font-medium text-(--jp-vermilion)-content transition-all hover:bg-(--jp-vermilion)/90"
            >
              保存配置
            </button>
          </div>
