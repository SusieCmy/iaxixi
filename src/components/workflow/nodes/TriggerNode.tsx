...
function TriggerNode({ data }: TriggerNodeProps) {
  ...
  return (
    <div className="group relative">
      ...
      {/* 标签提示 - 始终显示在底部 */}
      <div className="-bottom-10 -translate-x-1/2 absolute left-1/2 whitespace-nowrap rounded-lg border border-(--jp-mist) bg-(--jp-cream) px-3 py-1.5 font-medium text-(--jp-ink) text-xs shadow-sm">
      ...
      {!hasConnectedNode && (
        <div className="-right-4 absolute bottom-1">
          <button
            ...
            className="flex h-6 w-6 items-center justify-center rounded-full border border-(--jp-mist) bg-(--jp-cream) text-(--jp-ink)/60 shadow-sm transition-all hover:scale-110 hover:border-(--jp-vermilion) hover:bg-(--jp-vermilion) hover:text-white"
