    ...
    return (
      ...
      >
        ...
        <div className="mt-1 flex-shrink-0">
          ...
          >
            {message.role === 'user' ? (
              <User className="h-4 w-4 text-primary-content sm:h-5 sm:w-5" strokeWidth={2.5} />
            ) : (
              <Bot className="h-4 w-4 text-secondary-content sm:h-5 sm:w-5" strokeWidth={2.5} />
            )}
          </div>
        </div>
        ...
        >
          ...
          >
            ...
            <span className="text-[10px] text-base-content/50 sm:text-xs">
          ...
          >
            {showLoading ? (
              <div className="flex items-center gap-2">
                ...
                <span className="text-base-content/60 text-xs sm:text-sm">思考中...</span>
              </div>
                  ...
                  components={{
                    code({ node, inline, className, children, ...props }: any) {
                      ...
                      return !inline && match ? (
                        ...
                        <code
                          className="rounded bg-base-300 px-1 py-0.5 text-[0.85em] text-primary sm:px-1.5 sm:text-[0.9em]"
                          {...props}
                        >
                          {children}
                        </code>
          ...
          {message.role === 'assistant' && onRegenerate && message.content && !isStreaming && (
            <button
              ...
              className="mt-2 flex items-center gap-1 text-base-content/50 text-xs transition-colors hover:text-primary"
            >
