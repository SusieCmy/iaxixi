...
class WorkflowErrorBoundary extends Component<Props, State> {
  ...
  render() {
    if (this.state.hasError) {
      ...
      return (
        <div className="flex h-screen w-screen items-center justify-center bg-(--jp-cream)">
          <div className="max-w-md rounded-xl border border-error/20 bg-(--jp-paper)/50 p-8 text-center shadow-lg">
            {/* 错误图标 */}
            ...
            {process.env.NODE_ENV === 'development' && this.state.error && (
              <div className="mb-6 max-h-40 overflow-auto rounded-lg bg-(--jp-mist) p-4 text-left">
                <p className="mb-2 font-mono text-error text-sm">
                  <strong>Error:</strong> {this.state.error.message}
                </p>
                <pre className="overflow-x-auto font-mono text-(--jp-ink)/60 text-xs">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-lg bg-(--jp-vermilion) px-6 py-3 font-medium text-(--jp-vermilion)-content transition-all hover:scale-105 hover:bg-(--jp-vermilion)/90"
            >
